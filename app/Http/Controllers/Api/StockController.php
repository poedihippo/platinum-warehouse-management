...
class StockController extends Controller
{
    ...
    public function index()
    {
        ...
        $stockProductUnits = QueryBuilder::for(StockProductUnit::select('stock_product_units.id', 'stock_product_units.qty', 'stock_product_units.product_unit_id', 'stock_product_units.warehouse_id')
            ...
            ->allowedFilters([
                ...
                AllowedFilter::scope('is_product_unit_qr', 'whereProductUnitQr'),
    ...
    public function show(string $id, Request $request)
    {
        ...
        $includeDeliveryOrder = str_contains($include, 'deliveryOrder');
    ...
    public function destroy(Stock $stock)
    {
        ...
        if ($stock->childs()->count() > 0) {
            return response()->json(['message' => 'Tidak dapat menghapus stock parent'], 400);
        }
        if ($stock->salesOrderItems()->whereNotReturned()->count() > 0) {
            return response()->json(['message' => 'Tidak dapat menghapus stock. Stock sudah masuk di Sales Order'], 400);
        }
    ...
    public function groupingByScan(GroupingByScanRequest $request)
    {
        ...
        DB::transaction(function () use ($request, $stockProductUnit, $description) {
            ...
            Stock::whereIn('id', $request->ids)->where('stock_product_unit_id', $stockProductUnit->id)->whereNull('parent_id')->update($data);
        });
    ...
    public function ungrouping(string $id)
    {
        ...
        if ($stock->childs->isEmpty()) {
            return response()->json(['message' => 'Stock bukan grouping / tidak memiliki childs'], 400);
        }
