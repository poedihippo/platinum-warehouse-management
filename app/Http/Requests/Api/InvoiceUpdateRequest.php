...
class InvoiceUpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            'voucher_codes.*' => ['required', 'string', function (string $attribute, mixed $value, Closure $fail) use ($salesOrder) {
                $voucher = Voucher::where('code', $value)->first();
                if (! $voucher) {
                    return $fail('Voucher tidak ditemukan!');
                }
                if ($voucher->is_used && ! $salesOrder->vouchers()->where('voucher_id', $voucher->id)->exists()) {
